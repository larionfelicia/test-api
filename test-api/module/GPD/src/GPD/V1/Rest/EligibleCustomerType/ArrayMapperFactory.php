...
class ArrayMapperFactory
{
    public function __invoke($services)
    {
        if (!$services->has('Config')) {
            throw new DomainException('Cannot create GPD\V1\Rest\EligibleCustomerType\ArrayMapper; missing Config dependency');
        }

        $config = $services->get('Config');
        if (! isset($config['eligibleCustomerType']['array_mapper_path'])) {
            throw new DomainException('Cannot create SGPD\V1\Rest\EligibleCustomerType\ArrayMapper; missing product.array_mapper_path configuration');
        }

        $path = $config['eligibleCustomerType']['array_mapper_path'];
        if (! file_exists($path)) {
            throw new DomainException(sprintf(
                'Cannot create GPD\V1\Rest\EligibleCustomerType\ArrayMapper; path "%s" does not exist',
                $path
            ));
        }
        ...
        if (! is_array($data)) {
           throw new DomainException(sprintf(
                'Cannot create GPD\V1\Rest\EligibleCustomerType\ArrayMapper; file "%s" does not return an array',
                $path
            ));
        }
