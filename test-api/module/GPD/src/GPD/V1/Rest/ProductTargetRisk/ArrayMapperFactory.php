...
class ArrayMapperFactory
{
    public function __invoke($services)
    {
        if (!$services->has('Config')) {
            throw new DomainException('Cannot create GPD\V1\Rest\ProductTargetRisk\ArrayMapper; missing Config dependency');
        }

        $config = $services->get('Config');
        if (! isset($config['productTargetRisk']['array_mapper_path'])) {
            throw new DomainException('Cannot create SGPD\V1\Rest\ProductTargetRisk\ArrayMapper; missing productTargetRisk.array_mapper_path configuration');
        }

        $path = $config['productTargetRisk']['array_mapper_path'];
        if (! file_exists($path)) {
            throw new DomainException(sprintf(
                'Cannot create GPD\V1\Rest\ProductTargetRisk\ArrayMapper; path "%s" does not exist',
                $path
            ));
        }
        ...
        if (! is_array($data)) {
           throw new DomainException(sprintf(
                'Cannot create GPD\V1\Rest\ProductTargetRisk\ArrayMapper; file "%s" does not return an array',
                $path
            ));
        }
