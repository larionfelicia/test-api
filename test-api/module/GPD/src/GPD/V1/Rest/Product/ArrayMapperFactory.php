...
class ArrayMapperFactory
{
    public function __invoke($services)
    {
        if (!$services->has('Config')) {
            throw new DomainException('Cannot create GPD\V1\Rest\Product\ArrayMapper; missing Config dependency');
        }

        $config = $services->get('Config');
        if (! isset($config['product']['array_mapper_path'])) {
            throw new DomainException('Cannot create SGPD\V1\Rest\Product\ArrayMapper; missing product.array_mapper_path configuration');
        }

        $path = $config['product']['array_mapper_path'];
        if (! file_exists($path)) {
            throw new DomainException(sprintf(
                'Cannot create GPD\V1\Rest\Product\ArrayMapper; path "%s" does not exist',
                $path
            ));
        }
        ...
        if (! is_array($data)) {
           throw new DomainException(sprintf(
                'Cannot create GPD\V1\Rest\Product\ArrayMapper; file "%s" does not return an array',
                $path
            ));
        }
