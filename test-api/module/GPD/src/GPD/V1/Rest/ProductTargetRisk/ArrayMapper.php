...
class ArrayMapper implements MapperInterface
{
    ...
    {   
        ...
        $this->entityPrototype = new ProductTargetRiskEntity;
    ...
    {   
        ...
        foreach ($this->data as $key => $value) {
            if ($value['productId'] == $data['productId']) {
                throw new InvalidArgumentException(sprintf(
                'Invalid data provided, target market and risk situation already exists for this product',
                __METHOD__
            ));
            }
        }
    ...
    public function fetch($id)
    {
        ...
        throw new DomainException('Target Market and Risk situation not found for product id '.$id, 404);
    }
    ...
    {
        return new ProductTargetRiskCollection($this->createCollection());
    }
    ...
    public function update($id, $data)
    {
        ...
        if (! array_key_exists($id, $this->data)) {
            throw new DomainException('Cannot update; no such target market and risk situation', 404);
        }
    ...
    {
        if (! array_key_exists($id, $this->data)) {
            throw new DomainException('Cannot delete; no such target market and risk situation', 404);
        }
